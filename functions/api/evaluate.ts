...
export async function onRequestPost(context: PagesContext) {
  try {
    ...
    if (response.positions.length) await recordEvent(context.env, 'DEBT_FOUND', scope, sessionId, String(body.source ?? 'direct'));
    if (response.status === 'LIVE_QUOTE_READY') await recordEvent(context.env, 'QUOTE_READY', scope, sessionId, String(body.source ?? 'direct'));
